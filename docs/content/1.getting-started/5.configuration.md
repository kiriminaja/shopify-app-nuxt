---
title: Configuration
description: All module options and runtime configuration for shopify-nuxt.
navigation:
  icon: i-lucide-settings
seo:
  title: Configuration — shopify-nuxt
  description: Complete reference for shopify-nuxt module options, configureShopify() runtime config, and navigation links.
---

## Module Options

Configure `shopify-nuxt` in your `nuxt.config.ts` under the `shopify` key:
...
Configure sidebar navigation via the `navLinks` option. These are rendered by `<ShopifyAppProvider>` as the [App Bridge nav menu](https://shopify.dev/docs/api/app-home/app-bridge-web-components/app-nav):
