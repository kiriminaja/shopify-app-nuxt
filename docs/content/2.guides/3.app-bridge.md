...
Use `useShopifyFetch()` for API calls that automatically include the Shopify session token. It works on both client and server:

- **Client**: Fetches a session token via App Bridge and sets the `Authorization` header
- **Server**: Forwards the incoming `Authorization` header from the request
